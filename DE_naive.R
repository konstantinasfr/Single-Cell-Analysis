# DEPENDENCIES: total_labels_naive
# must execute first seurat_svm.R and naive_bayes.R files 
...
cardio <- filter(total_labels_naive, x == "Cardiomyocytes")
...
fibro <- filter(total_labels_naive, x == "Fibroblasts" )
#fibro["Cells"] <- attr(cardio, "row.names")
fibro_seurat <- attr(fibro, "row.names")

endo <- filter(total_labels_naive, x == "Endothelial")
endo_seurat <- attr(endo, "row.names")

immun <- filter(total_labels_naive, x == "Immune" )
